...
# 检查并拉起无头 IK 求解服务节点 (若尚未运行)
if ! pgrep -f "g1_ik_node.py" > /dev/null 2>&1; then

    echo "[提示] 启动后台 Headless 10-DoF IK 求解服务节点 (deploy/solver/g1_ik_node.py)..."
    python3 "$DIR/deploy/solver/g1_ik_node.py" &
    IK_PID=$!
    trap "kill $IK_PID 2>/dev/null || true" EXIT
    sleep 0.5
fi

python3 "$DIR/deploy/visualizer/web/server.py" --port "$PORT"
