...
--------------------------------------------------------------------------------------------------------------
7-DoF 左臂自由空间规划               left_arm        RRTConnect  成功       66.48 ms     8 个点     0.69 s
10-DoF 左臂+腰部协同规划             left_arm_torso  RRTConnect  成功       26.50 ms     9 个点     0.76 s
动态立方体障碍物避障 (RRTConnect)    left_arm        RRTConnect  成功       321.76 ms    19 个点    1.72 s
动态立方体障碍物避障 (RRT*)          left_arm        RRTstar     成功       5017.85 ms   19 个点    1.71 s
==============================================================================================================
```

> **注**：RRTConnect 算法在几十毫秒到几百毫秒内即可极速收敛，适用于工业节拍要求高的抓取场景；RRT* 具有渐进最优性，适合对空间轨迹能耗与长度要求极高的精细避障场景。
